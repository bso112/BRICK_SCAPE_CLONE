...
_int CGameEndPanel::Update_GameObject(_double _timeDelta)
{
	...
	if (m_bSceneChange)
	{
		...
		pEngineMgr->SetUp_CurrentScene(CScene_Loading::Create(m_pGraphic_Device, (SCENEID)(pManagement->Get_CurrScene() + 1)), pManagement->Get_CurrScene() + 1);

		pManagement->Clear_Component_Manager(pManagement->Get_CurrScene() - 1);
		pManagement->Clear_Object_Manager(pManagement->Get_CurrScene() - 1);

		CGameManager::Get_Instance()->Set_IsGameStart(false);
		CGameManager::Get_Instance()->Set_IsGoal(false);
